 ...
 *  baud = baud rate
 */
enum sunpci_board_num_t {
	sun_4port = 0,
};

static struct pciserial_board sunpciserial_boards[] __devinitdata = {
	[sun_4port] = {
		.flags		= FL_BASE0,
		.num_ports	= 4,
		.base_baud	= 115200,
		  ...
		  struct uart_port *port, int idx)
{
	...
	printk(KERN_INFO "Exar PCIe device 0x%x--type:%d\n", priv->dev->device,port->type);
	#if 0
...
{
	int ret =(quirk_id == PCI_ANY_ID || quirk_id == dev_id);
	printk("ret========$%p=======%x,%x,%x\r\n",pci_serial_quirks,ret,quirk_id,dev_id);
	return ret;
}
...
static struct pci_serial_quirk *find_quirk(struct pci_dev *dev)
{
	...
	struct pci_serial_quirk *quirk =NULL;

	for (quirk = pci_serial_quirks; ; quirk++)
		{
		 if (quirk_id_matches(quirk->vendor, dev->vendor) &&quirk_id_matches(quirk->device, dev->device))		    
		 	break;
		}
	return quirk;
}
...
{
	return 0;//readb(up->port.membase + offset);
}
...
static struct uart_xr_port *serialxr_find_match_or_unused(struct uart_port *port)
{
	 ...
	 */
	 printk("uart-------%d,%d\r\n",serialxr_ports[i].port.type,port->type);
...
int serialxr_register_port(struct uart_port *port, unsigned short deviceid, unsigned char channelnum)
{
	...
	if (uart) {
		...
		ret = uart_add_one_port(&sun_uart_driver, &uart->port);
		printk("ret===============%d\r\n",ret);
#if 0
		if (ret == 0)
		{
...
init_one_xrpciserialcard(struct pci_dev *dev, const struct pci_device_id *ent)
{
	...
	if (ent->driver_data >= ARRAY_SIZE(sunpciserial_boards)) {
		printk(KERN_INFO "pci_init_one: invalid driver_data: %ld\n",
			ent->driver_data);
		return -EINVAL;
	}

	board = &sunpciserial_boards[ent->driver_data];
	...
	printk("board2-------------%d,rc:%p,r\n",board->num_ports,quirk);
	...
	memset(priv, 0, sizeof(struct serial_private) +
			sizeof(unsigned int) * nr_ports);
	//dev2 = pci_get_device(0X8086, 0X100F, dev2);
	
	printk("erial_port.uartclk ----%d,irq:%d,dev:%x\r\n",serial_port.uartclk ,dev->irq,dev->dev);
	...
	/*if((priv->dev->device == 0x152)	||(priv->dev->device == 0x154)||(priv->dev->device == 0x158))
		...
		serial_port.uartclk = board->base_baud ;
	...
	for (i = 0; i < nr_ports; i++) {
		if (quirk->setup(priv, board, &serial_port, i))//pci_default_setup
			break;
...
#endif
	uart_remove_one_port(&sun_uart_driver, &uart->port);
	uart->port.dev = NULL;	
	mutex_unlock(&serial_mutex);
}
...
static int __init serialxr_init(void)
{
	...
	ret = uart_register_driver(&sun_uart_driver);
	if (ret)
		return ret;
	...
	if (ret < 0)
		uart_unregister_driver(&sun_uart_driver);
